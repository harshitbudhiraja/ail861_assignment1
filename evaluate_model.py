...
class ModelEvaluator:
    ...
    def generate_continuations(self, prompts: List[str], max_length: int = 50,
        ...
        for prompt in tqdm(prompts, desc="Generating continuations"):
            start_tokens = torch.tensor([self.processor.text_to_ids(prompt)], device=self.device)
            with torch.no_grad():
    ...
    def evaluate_on_dataset(self, test_file: str, num_samples: int = 100,
        ...
        for story in stories:
            ...
            if len(words) > 20:
                ...
                reference = ' '.join(words[20:])
                prompts.append(prompt)
                references.append(reference)
        print(f"Generated {len(prompts)} prompt-reference pairs")
        ...
        results = {
            ...
            'generated_texts': generated_texts[:5],
